<script lang="ts">
  ...
  import type { StudentDetailProps } from '../types/students.types';
  import type { StudentInfo as OldStudentInfo } from '$lib/utils/user';

  const { student, homework, exercises, isTeacherView }: StudentDetailProps =
    $props();
  ...
  const handleHomeworkAssigned = async () => {
    ...
    toast.success(
      `Домашнее задание назначено студенту ${formatStudentFullName(student.firstName, student.lastName)}!`
    );
  ...
  const overallStats = $derived(() => calculateOverallHomeworkStats(homework));
</script>

<svelte:head>
  <title
    >Домашние задания: {formatStudentFullName(
      student.firstName,
      student.lastName
    )} - Finch</title
  >
