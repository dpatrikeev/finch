<script lang="ts">
  ...
  import type { HomeworkWithProgress } from '../types/homework.types';
  import { CircleCheck, Clock, CircleAlert, ChartBar } from 'lucide-svelte';
  ...
  const getExerciseStatus = (exerciseId: string, hw: HomeworkWithProgress) => {
    ...
    // Используем статус из exercises_status, который уже учитывает глобальную историю ответов
    if (status.completed) {
        ...
        {#if showDetails}
          <Button
            ...
            href="/students/{studentId ||
              'current-user'}/homework/{homework.id}"
      ...
      <div class="grid gap-3">
        {#each homework.exercises as exerciseId}
          ...
          >
            ...
            >
              {#if exerciseStatus?.attempts > 0 && showDetails}
