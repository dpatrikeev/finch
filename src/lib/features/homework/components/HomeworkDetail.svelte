<script lang="ts">
  ...
  import type { HomeworkStats } from '../types/homework.types';
  import {
    CircleCheck,
    CircleX,
    Clock,
  ...
  import EditHomeworkButton from './buttons/EditHomeworkButton.svelte';
  ...
  const { stats, student, exercises }: Props = $props();
...
<div class="container mx-auto px-4 py-4 md:py-8 max-w-6xl">
  ...
  <div class="bg-white rounded-lg shadow-md border p-4 md:p-6 mb-4 md:mb-6">
      ...
      <div class="md:text-right">
        ...
        >
          ...
          <EditHomeworkButton
            ...
            variant="outline"
            size="sm"
            class="self-start text-xs md:text-sm px-3 py-1 md:px-4 md:py-2"
    ...
    <div class="flex flex-col gap-2 md:text-right">
      {#if stats.overall_progress.completion_percentage === 100}
        <Badge
      ...
      <div class="text-xs md:text-sm text-gray-500">
        {stats.overall_progress.completed_exercises} из {stats.overall_progress
    ...
    >
      ...
      <div class="text-center">
        ...
        <div class="text-lg md:text-2xl font-bold text-green-600">
          {getAccuracy(
            stats.overall_progress.correct_attempts,
            stats.overall_progress.total_attempts
          )}%
        </div>
      ...
      <div class="text-center">
        ...
        <div class="text-lg md:text-2xl font-bold text-purple-600">
          {#if stats.homework.started_at}
            {Math.ceil(
              (new Date().getTime() -
                new Date(stats.homework.started_at).getTime()) /
                (1000 * 60 * 60 * 24)
            )}
          {:else}
            0
    ...
    <!-- Временные метки -->
    {#if stats.homework.started_at || stats.homework.completed_at}
      <div
      ...
      >
        {#if stats.homework.started_at}
          <div>
            ...
            {formatFullDate(stats.homework.started_at)}
          </div>
        {/if}
        {#if stats.homework.completed_at}
          <div>
            <span class="font-medium">Завершено:</span>
            {formatFullDate(stats.homework.completed_at)}
          </div>
        {/if}
      </div>
    {/if}
  ...
  <div class="bg-white rounded-lg shadow-md border p-4 md:p-6">
    <h3 class="text-lg md:text-xl font-semibold mb-4 md:mb-6">
      <span>Статистика упражнений</span>
    </h3>

    <div class="space-y-4">
      {#if !stats.exercises_stats || stats.exercises_stats.length === 0}
        <div class="text-center py-8 text-gray-500">
          <p>Нет данных по упражнениям</p>
        </div>
      {:else}
        {#each stats.exercises_stats as exerciseStat}
          {@const StatusIcon = getStatusIcon(
            exerciseStat.is_completed,
            exerciseStat.total_attempts
          )}
          ...
          <div class="border rounded-lg p-3 md:p-4">
            ...
            {#if exerciseStat.answers_history.length > 0}
              <details class="mt-3">
                <summary
                ...
                <div class="mt-3 space-y-2 max-h-48 overflow-y-auto">
                  {#each exerciseStat.answers_history as answer}
                    ...
                    >
                      ...
                      <span
                        class="text-xs text-gray-500 self-start md:self-auto"
                      >
